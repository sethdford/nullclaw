...
{
    (void)system_prompt; (void)system_prompt_len; (void)message; (void)message_len;
...
static bool mock_toolcall_supports_native_tools(void *ctx) { (void)ctx; return false; }
static const char *mock_toolcall_get_name(void *ctx) {
    return ((mock_toolcall_provider_t *)ctx)->name;
}
static void mock_toolcall_deinit(void *ctx, sc_allocator_t *alloc) { (void)ctx; (void)alloc; }
...
static sc_provider_t mock_toolcall_provider_create(sc_allocator_t *alloc, mock_toolcall_provider_t *ctx) {
    (void)alloc;
    ctx->call_count = 0;
...
/* Mock tool */
typedef struct mock_sec_tool { const char *name; } mock_sec_tool_t;
...
static const char *mock_sec_tool_name(void *ctx) { return ((mock_sec_tool_t *)ctx)->name; }
static const char *mock_sec_tool_desc(void *ctx) { (void)ctx; return "mock"; }
static const char *mock_sec_tool_params(void *ctx) { (void)ctx; return "{}"; }
static void mock_sec_tool_deinit(void *ctx, sc_allocator_t *alloc) { (void)ctx; (void)alloc; }
...
static void test_locked_mode_blocks_all_tools(void) {
    ...
    sc_error_t err = sc_agent_from_config(&agent, &alloc, prov,
        tools, 1, NULL, NULL, NULL, NULL,
        "gpt-4", 5, "openai", 6, 0.7,
        ".", 1, 25, 50, false, SC_AUTONOMY_LOCKED, NULL, 0, NULL);
...
static void test_supervised_forces_approval_on_all_tools(void) {
    ...
    sc_error_t err = sc_agent_from_config(&agent, &alloc, prov,
        tools, 1, NULL, NULL, NULL, NULL,
        "gpt-4", 5, "openai", 6, 0.7,
        ".", 1, 25, 50, false, SC_AUTONOMY_SUPERVISED, NULL, 0, NULL);
    SC_ASSERT_EQ(err, SC_OK);
...
static void test_autonomous_passes_through_to_policy_engine(void) {
    ...
    sc_policy_match_t m = { .tool = "shell", .args_contains = NULL, .session_cost_gt = 0, .has_cost_check = false };
...
static void test_policy_deny_blocks_tool_in_agent_flow(void) {
    ...
    sc_error_t err = sc_agent_from_config(&agent, &alloc, prov,
        tools, 1, NULL, NULL, NULL, NULL,
        "gpt-4", 5, "openai", 6, 0.7,
        ".", 1, 25, 50, false, SC_AUTONOMY_AUTONOMOUS, NULL, 0, NULL);
    SC_ASSERT_EQ(err, SC_OK);
