...
- minimal binary size (~528 KB release with LTO)
...
Current scale: **597 source + header files, ~103K lines of C, ~44K lines of tests, 3,127 tests, 33 channels**.
...
2. **Binary size and memory are hard product constraints**
   ...
   - Current release binary: ~528 KB (all features with LTO).
...
5. **All 3,127+ tests must pass at zero ASan errors**
