...
static uint32_t lookup_table(const max_entry_t *tbl, size_t tbl_len,
...
static uint32_t infer_from_pattern(const char *model_id, size_t len) {
    if (len >= 4 && contains_str(model_id, len, "k2p5")) return 32768;
    if (starts_with_ic(model_id, len, "kimi-coding") ||
        starts_with_ic(model_id, len, "kimi-k2")) return 32768;
    if (starts_with_ic(model_id, len, "nvidia/")) return 4096;
    if (starts_with_ic(model_id, len, "claude-") ||
        starts_with_ic(model_id, len, "gpt-") ||
        starts_with_ic(model_id, len, "o1") ||
        starts_with_ic(model_id, len, "o3") ||
        starts_with_ic(model_id, len, "gemini-") ||
        starts_with_ic(model_id, len, "deepseek-")) return 8192;
    return 0;
}
