...
static sc_error_t append(sc_allocator_t *alloc, char **buf, size_t *len, size_t *cap,
...
{
    if (!alloc || !config || !out || !out_len) return SC_ERR_INVALID_ARGUMENT;
    ...
    if (config->workspace_dir && config->workspace_dir_len > 0) {
        ...
        if (err != SC_OK) goto fail;
        err = append(alloc, &buf, &len, &cap,
            config->workspace_dir, config->workspace_dir_len);
        if (err != SC_OK) goto fail;
        err = append(alloc, &buf, &len, &cap, "\n\n", 2);
        if (err != SC_OK) goto fail;
    }
    ...
    if (config->provider_name && config->provider_name_len > 0) {
        ...
        int n = snprintf(line, sizeof(line), "Provider: %.*s\n",
            (int)config->provider_name_len, config->provider_name);
        if (n > 0) {
    ...
    if (config->model_name && config->model_name_len > 0) {
        ...
        int n = snprintf(line, sizeof(line), "Model: %.*s\n",
            (int)config->model_name_len, config->model_name);
        if (n > 0) {
            err = append(alloc, &buf, &len, &cap, line, (size_t)n);
            if (err != SC_OK) goto fail;
        }
    ...
    if (config->memory_context && config->memory_context_len > 0) {
        err = append(alloc, &buf, &len, &cap,
            config->memory_context, config->memory_context_len);
        if (err != SC_OK) goto fail;
        err = append(alloc, &buf, &len, &cap, "\n\n", 2);
        if (err != SC_OK) goto fail;
    } else {
    ...
    if (config->autonomy_level == 0) {
        ...
        err = append(alloc, &buf, &len, &cap,
            "## Rules\n\nYou are in supervised mode. Ask before running destructive or high-impact commands.\n\n", 89);
        if (err != SC_OK) goto fail;
    ...
    if (config->custom_instructions && config->custom_instructions_len > 0) {
        err = append(alloc, &buf, &len, &cap,
            config->custom_instructions, config->custom_instructions_len);
        if (err != SC_OK) goto fail;
