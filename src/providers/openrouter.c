    ...
    sc_chat_response_t *out)
{
    ...
    if (!orc || !request || !out) return SC_ERR_INVALID_ARGUMENT;
    (void)model;
    (void)model_len;
    (void)temperature;
    ...
    char auth_buf[512];
    int n = snprintf(auth_buf, sizeof(auth_buf), "Bearer %.*s", (int)orc->api_key_len, orc->api_key);
...
static bool openrouter_supports_native_tools(void *ctx) { (void)ctx; return true; }
static const char *openrouter_get_name(void *ctx) { (void)ctx; return "openrouter"; }
static void openrouter_deinit(void *ctx, sc_allocator_t *alloc) {
    (void)alloc;
    sc_openrouter_ctx_t *orc = (sc_openrouter_ctx_t *)ctx;
    if (orc && orc->api_key) { free(orc->api_key); free(orc); }
}
...
sc_error_t sc_openrouter_create(sc_allocator_t *alloc,
    ...
    sc_provider_t *out)
{
    ...
    if (!orc) return SC_ERR_OUT_OF_MEMORY;
    if (api_key && api_key_len > 0) {
        orc->api_key = (char *)malloc(api_key_len + 1);
        if (!orc->api_key) { free(orc); return SC_ERR_OUT_OF_MEMORY; }
