...
static void entry_free(sc_allocator_t *a, sc_skill_registry_entry_t *e) {
    if (!a || !e) return;
    if (e->name) a->free(a->ctx, e->name, strlen(e->name) + 1);
    if (e->description) a->free(a->ctx, e->description, strlen(e->description) + 1);
    if (e->version) a->free(a->ctx, e->version, strlen(e->version) + 1);
    if (e->author) a->free(a->ctx, e->author, strlen(e->author) + 1);
    if (e->url) a->free(a->ctx, e->url, strlen(e->url) + 1);
    if (e->tags) a->free(a->ctx, e->tags, strlen(e->tags) + 1);
    memset(e, 0, sizeof(*e));
}
...
sc_error_t sc_skill_registry_search(sc_allocator_t *alloc, const char *query,
    ...
    (void)query;
    if (!alloc || !out_entries || !out_count) return SC_ERR_INVALID_ARGUMENT;
    *out_entries = NULL;
    *out_count = 0;

    sc_skill_registry_entry_t *arr = (sc_skill_registry_entry_t *)alloc->alloc(alloc->ctx,
        2 * sizeof(sc_skill_registry_entry_t));
    ...
    arr[0].url = sc_strdup(alloc, "https://github.com/seaclaw/skill-registry/tree/main/skills/code-review");
    ...
    arr[1].url = sc_strdup(alloc, "https://github.com/seaclaw/skill-registry/tree/main/skills/email-digest");
...
void sc_skill_registry_entries_free(sc_allocator_t *alloc,
    ...
    for (size_t i = 0; i < count; i++) entry_free(alloc, &entries[i]);
...
static bool matches_query(const char *q, const char *name, const char *desc, const char *tags) {
    ...
    if (!name && !desc && !tags) return false;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (size_t j = 0; j + ql <= vl; j++) {
            if (strncasecmp(v + j, q, ql) == 0) return true;
        }
    }
    return false;
}
...
sc_error_t sc_skill_registry_search(sc_allocator_t *alloc, const char *query,
    ...
    *out_count = 0;
    ...
    size_t cap = 16;
    sc_skill_registry_entry_t *arr = (sc_skill_registry_entry_t *)alloc->alloc(alloc->ctx,
        cap * sizeof(sc_skill_registry_entry_t));
    if (!arr) {
    ...
    for (size_t i = 0; i < root->data.array.len && count < 256; i++) {
        ...
        if (!obj || obj->type != SC_JSON_OBJECT) continue;
        ...
        if (tags_val) {
            if (tags_val->type == SC_JSON_STRING && tags_val->data.string.ptr)
                ...
                char tags_buf[256];
                size_t pos = 0;
                for (size_t j = 0; j < tags_val->data.array.len && pos < sizeof(tags_buf) - 2; j++) {
                    sc_json_value_t *t = tags_val->data.array.items[j];
                    if (t && t->type == SC_JSON_STRING && t->data.string.ptr) {
                        if (j > 0) { tags_buf[pos++] = ','; tags_buf[pos++] = ' '; }
                        size_t len = t->data.string.len;
                        if (pos + len >= sizeof(tags_buf)) len = sizeof(tags_buf) - pos - 1;
                        memcpy(tags_buf + pos, t->data.string.ptr, len);
                        pos += len;
                    }
                }
                tags_buf[pos] = '\0';
                if (pos > 0) tags_str = tags_buf;  /* only for match, not stored */
            }
        }
        if (!name) continue;
        ...
        if (count >= cap) {
            ...
            sc_skill_registry_entry_t *n = (sc_skill_registry_entry_t *)alloc->realloc(alloc->ctx,
                arr, cap * sizeof(sc_skill_registry_entry_t), new_cap * sizeof(sc_skill_registry_entry_t));
            if (!n) {
                for (size_t k = 0; k < count; k++) entry_free(alloc, &arr[k]);
...
static void url_tree_to_raw(const char *tree_url, char *raw_base, size_t raw_len) {
    ...
    int n = snprintf(raw_base, raw_len,
        "https://raw.githubusercontent.com/%.*s/%.*s/%s",
        (int)org_repo, p, (int)branch_len, branch_start, path);
    if (n < 0 || (size_t)n >= raw_len) raw_base[0] = '\0';
}

sc_error_t sc_skill_registry_install(sc_allocator_t *alloc, const char *name) {
    if (!alloc || !name || !name[0]) return SC_ERR_INVALID_ARGUMENT;
    ...
    if (lb > 0 && raw_base[lb - 1] != '/') {
        if (lb + 1 < sizeof(raw_base)) { raw_base[lb] = '/'; raw_base[lb + 1] = '\0'; }
    }
    ...
    char skill_path[512];
    int n = snprintf(skill_path, sizeof(skill_path), "%s/.seaclaw/skills/%s.skill.json", home, name);
    ...
    if (n > 0 && (size_t)n < sizeof(skill_md_url)) {
        ...
        if (sc_http_get(alloc, skill_md_url, NULL, &md_resp) == SC_OK && md_resp.body && md_resp.body_len > 0) {
            char md_path[512];
...
sc_error_t sc_skill_registry_uninstall(const char *name) {
    if (!name || !name[0]) return SC_ERR_INVALID_ARGUMENT;
...
sc_error_t sc_skill_registry_update(sc_allocator_t *alloc) {
...
#if !defined(_WIN32)
    {
        ...
        while ((e = readdir(d)) != NULL) {
            ...
            if (nlen < 12 || strcmp(e->d_name + nlen - 11, ".skill.json") != 0) continue;
            char skill_name[260];
            if (nlen <= 11) continue;
            size_t name_len = nlen - 11;
            if (name_len >= sizeof(skill_name)) continue;
