...
#define SC_MEMORY_RECALL_PARAMS "{\"type\":\"object\",\"properties\":{\"query\":{\"type\":\"string\"},\"limit\":{\"type\":\"number\"}},\"required\":[\"query\"]}"
    ...
    sc_tool_result_t *out)
{
    ...
    *out = sc_tool_result_ok_owned(msg, 20);
    ...
    if (limit_val < 0 || limit_val > SC_MEMORY_LIMIT_MAX) limit_val = 10;
    size_t limit = (size_t)limit_val;
    sc_memory_entry_t *entries = NULL;
    size_t count = 0;
    sc_error_t err = c->memory->vtable->recall(c->memory->ctx, alloc,
        query, strlen(query), limit, NULL, 0, &entries, &count);
    if (err != SC_OK) {
...
static const char *memory_recall_name(void *ctx) { (void)ctx; return SC_MEMORY_RECALL_NAME; }
static const char *memory_recall_description(void *ctx) { (void)ctx; return SC_MEMORY_RECALL_DESC; }
static const char *memory_recall_parameters_json(void *ctx) { (void)ctx; return SC_MEMORY_RECALL_PARAMS; }
static void memory_recall_deinit(void *ctx, sc_allocator_t *alloc) { (void)alloc; if (ctx) free(ctx); }

static const sc_tool_vtable_t memory_recall_vtable = {
    .execute = memory_recall_execute, .name = memory_recall_name,
    .description = memory_recall_description, .parameters_json = memory_recall_parameters_json,
...
sc_error_t sc_memory_recall_create(sc_allocator_t *alloc,
    ...
    sc_tool_t *out)
{
    sc_memory_recall_ctx_t *c = (sc_memory_recall_ctx_t *)calloc(1, sizeof(*c));
