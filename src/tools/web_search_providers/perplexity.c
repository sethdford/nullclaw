    ...
    sc_tool_result_t *out)
{
    ...
    int bn = snprintf(body_buf, sizeof(body_buf),
        "{\"query\":\"%s\",\"max_results\":%d}", escaped, count);
    ...
    sc_error_t err = sc_http_post_json(alloc, PERPLEXITY_API_URL, auth_buf,
        body_buf, (size_t)bn, &resp);
    if (err != SC_OK) {
        *out = sc_tool_result_fail("Perplexity search request failed", 34);
        return SC_OK;
    }
    if (resp.status_code < 200 || resp.status_code >= 300) {
        sc_http_response_free(alloc, &resp);
        *out = sc_tool_result_fail("Perplexity API error", 21);
        return SC_OK;
    }
    ...
    for (int i = 0; i < max_r; i++) {
        ...
        const char *desc = sc_json_get_string(item, "snippet");
        if (!title) title = ""; if (!url) url = ""; if (!desc) desc = "";
