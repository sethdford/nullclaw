...
#define SC_MEMORY_STORE_PARAMS "{\"type\":\"object\",\"properties\":{\"key\":{\"type\":\"string\"},\"content\":{\"type\":\"string\"}},\"required\":[\"key\",\"content\"]}"
    ...
    sc_tool_result_t *out)
{
    ...
    *out = sc_tool_result_ok_owned(msg, 19);
    ...
    sc_error_t err = c->memory->vtable->store(c->memory->ctx,
        key, strlen(key), content, strlen(content),
        &cat, NULL, 0);
    if (err != SC_OK) {
...
static const char *memory_store_name(void *ctx) { (void)ctx; return SC_MEMORY_STORE_NAME; }
static const char *memory_store_description(void *ctx) { (void)ctx; return SC_MEMORY_STORE_DESC; }
static const char *memory_store_parameters_json(void *ctx) { (void)ctx; return SC_MEMORY_STORE_PARAMS; }
static void memory_store_deinit(void *ctx, sc_allocator_t *alloc) { (void)alloc; if (ctx) free(ctx); }

static const sc_tool_vtable_t memory_store_vtable = {
    .execute = memory_store_execute, .name = memory_store_name,
    .description = memory_store_description, .parameters_json = memory_store_parameters_json,
