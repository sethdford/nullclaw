...
static sc_error_t agent_query_execute(void *ctx, sc_allocator_t *alloc,
    const sc_json_value_t *args, sc_tool_result_t *out)
{
    ...
    if (id_d < 0) {
        *out = sc_tool_result_fail("missing agent_id", 16); return SC_OK;
    }
    uint64_t agent_id = (uint64_t)id_d;
    const char *message = sc_json_get_string(args, "message");
    if (!message) { *out = sc_tool_result_fail("missing message", 15); return SC_OK; }

#if SC_IS_TEST
    (void)c;
    char *msg = sc_sprintf(alloc, "{\"agent_id\":%llu,\"response\":\"test response\"}", (unsigned long long)agent_id);
    *out = sc_tool_result_ok_owned(msg, msg ? strlen(msg) : 0);
#else
    if (!c->pool) {
        *out = sc_tool_result_fail("agent pool not configured", 25); return SC_OK;
    }
    char *response = NULL;
    size_t response_len = 0;
    sc_error_t err = sc_agent_pool_query(c->pool, agent_id, message, strlen(message),
        &response, &response_len);
    if (err != SC_OK) {
        *out = sc_tool_result_fail("query failed", 12); return SC_OK;
    }
    if (response) {
...
static const char *agent_query_name(void *ctx) { (void)ctx; return TOOL_NAME; }
static const char *agent_query_desc(void *ctx) { (void)ctx; return TOOL_DESC; }
static const char *agent_query_params(void *ctx) { (void)ctx; return TOOL_PARAMS; }
static void agent_query_deinit(void *ctx, sc_allocator_t *alloc) {
    if (ctx) alloc->free(alloc->ctx, ctx, sizeof(agent_query_ctx_t));
}
