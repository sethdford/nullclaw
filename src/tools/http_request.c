...
static sc_error_t parse_headers(sc_allocator_t *alloc, const sc_json_value_t *headers_val,
    ...
    for (size_t i = 0; i < headers_val->data.object.len; i++) {
        ...
        if (!pair->key || !pair->value || pair->value->type != SC_JSON_STRING)
            continue;
...
static sc_error_t http_request_execute(void *ctx, sc_allocator_t *alloc,
    ...
    if (!method_valid(method)) {
        ...
        *out = e ? sc_tool_result_fail_owned(e, (size_t)n)
                 : sc_tool_result_fail("Unsupported method", 18);
        return SC_OK;
    }
...
static const char *http_request_name(void *ctx) {
    (void)ctx;
    return SC_HTTP_REQUEST_NAME;
}
static const char *http_request_desc(void *ctx) {
    (void)ctx;
    return SC_HTTP_REQUEST_DESC;
}
