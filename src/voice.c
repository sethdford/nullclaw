    ...
    const char *file_path, char **out_text, size_t *out_len)
{
    ...
    *out_len = 0;
...
#if SC_IS_TEST
    {
        ...
        if (!mock) return SC_ERR_OUT_OF_MEMORY;
        memcpy(mock, prefix, plen);
        if (fplen > 0) memcpy(mock + plen, file_path, fplen);
        mock[plen + fplen] = '\0';
        *out_text = mock;
        *out_len = plen + fplen;
        return SC_OK;
    ...
    const char *endpoint = config->stt_endpoint && config->stt_endpoint[0]
        ? config->stt_endpoint : SC_VOICE_STT_DEFAULT_ENDPOINT;
    const char *model = config->stt_model && config->stt_model[0]
        ? config->stt_model : SC_VOICE_STT_DEFAULT_MODEL;
    ...
    if (!auth_hdr) return SC_ERR_OUT_OF_MEMORY;
    ...
    if (argc >= 16) {
        ...
        alloc->free(alloc->ctx, file_arg, file_arg_cap);
        alloc->free(alloc->ctx, model_arg, model_arg_cap);
        if (lang_arg) alloc->free(alloc->ctx, lang_arg, lang_arg_cap);
        return SC_ERR_INVALID_ARGUMENT;
    }
    ...
    sc_run_result_free(alloc, &result);
    if (err != SC_OK) return SC_ERR_PARSE;
    ...
    sc_json_free(alloc, parsed);
    if (!text) return SC_ERR_OUT_OF_MEMORY;
...
{
    if (!alloc || !config || !out_audio || !out_audio_len) return SC_ERR_INVALID_ARGUMENT;
    *out_audio = NULL;
    *out_audio_len = 0;
    ...
    const char *endpoint = config->tts_endpoint && config->tts_endpoint[0]
        ? config->tts_endpoint : SC_VOICE_TTS_DEFAULT_ENDPOINT;
    const char *model = config->tts_model && config->tts_model[0]
        ? config->tts_model : SC_VOICE_TTS_DEFAULT_MODEL;
    const char *voice = config->tts_voice && config->tts_voice[0]
        ? config->tts_voice : SC_VOICE_TTS_DEFAULT_VOICE;

    /* Build JSON body: {"model":"tts-1","input":"<escaped>","voice":"alloy"} */
    sc_json_buf_t body_buf = {0};
    if (sc_json_buf_init(&body_buf, alloc) != SC_OK) return SC_ERR_OUT_OF_MEMORY;
    if (sc_json_buf_append_raw(&body_buf, "{\"model\":\"", 9) != SC_OK) goto tts_fail;
    if (sc_json_append_string(&body_buf, model, strlen(model)) != SC_OK) goto tts_fail;
    if (sc_json_buf_append_raw(&body_buf, "\",\"input\":", 10) != SC_OK) goto tts_fail;
    if (sc_json_append_string(&body_buf, text ? text : "", text_len) != SC_OK) goto tts_fail;
    if (sc_json_buf_append_raw(&body_buf, "\",\"voice\":\"", 10) != SC_OK) goto tts_fail;
    if (sc_json_append_string(&body_buf, voice, strlen(voice)) != SC_OK) goto tts_fail;
    if (sc_json_buf_append_raw(&body_buf, "\"}", 2) != SC_OK) goto tts_fail;
    ...
    /* argv: curl -s -X POST -H "Authorization: Bearer ..." -H "Content-Type: application/json" -d '...' url */
    const char *argv[12];
    size_t argc = 0;
    argv[argc++] = "curl";
    argv[argc++] = "-s";
