...
static const char *detail_truncate(const char *detail) {
    ...
    if (len > SC_OBSERVER_DETAIL_MAX)
        len = SC_OBSERVER_DETAIL_MAX;
...
static void log_record_event(void *ctx, const sc_observer_event_t *event) {
    ...
    case SC_OBSERVER_EVENT_AGENT_START:
        fprintf(f, "agent.start provider=%s model=%s\n", SC_STR(event->data.agent_start.provider),
                SC_STR(event->data.agent_start.model));
        break;
    case SC_OBSERVER_EVENT_LLM_REQUEST:
        fprintf(f, "llm.request provider=%s model=%s messages=%zu\n",
                SC_STR(event->data.llm_request.provider), SC_STR(event->data.llm_request.model),
    ...
    case SC_OBSERVER_EVENT_TOOL_CALL: {
        ...
        if (d)
            fprintf(f, "tool.call tool=%s duration_ms=%llu success=%s detail=%s\n",
                    SC_STR(event->data.tool_call.tool),
    ...
    case SC_OBSERVER_EVENT_CHANNEL_MESSAGE:
        fprintf(f, "channel.message channel=%s direction=%s\n",
                SC_STR(event->data.channel_message.channel),
                SC_STR(event->data.channel_message.direction));
        break;
    case SC_OBSERVER_EVENT_HEARTBEAT_TICK:
        fprintf(f, "heartbeat.tick\n");
        break;
    case SC_OBSERVER_EVENT_ERR:
        fprintf(f, "error component=%s message=%s\n", SC_STR(event->data.err.component),
                SC_STR(event->data.err.message));
        break;
    }
}
...
sc_observer_t sc_observer_log_stderr(void) {
    return sc_observer_log_create(NULL);
}
